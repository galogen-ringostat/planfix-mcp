...
- **ALL server-authored text is ALWAYS English — no Russian anywhere** (operator decision 2026-07-26, supersedes the earlier "data labels stay Russian" exception). This covers error messages, refusals, validation messages, corrective hints, tool descriptions, Zod `.describe()` strings, AND every rendered-output label and template (`status:`, `Tasks (N):`, `assignees:`, verdict lines, auto-split marks, …). Only DATA echoed from Planfix (task names, comments, user names) keeps its source language. Rationale: all server text is consumed by an LLM agent; it must match the working language, and English-only output is a public-release prerequisite. Migrate any remaining Russian strings whenever you touch the file that contains them; extend the existing no-Cyrillic test coverage (P3, 2026-07-24: schemas/descriptions) to rendered-output templates so the rule stays test-enforced.
